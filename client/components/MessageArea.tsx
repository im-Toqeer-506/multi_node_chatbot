...
type SearchStage = 'searching' | 'reading' | 'writing' | 'error';

interface SearchInfo {
    stages?: SearchStage[];
    query?: string;
    urls?: string[] | string | Record<string, unknown>;
    error?: string;
}
...
const SearchStages = ({ searchInfo }: SearchStagesProps) => {
    ...
    const urls = searchInfo.urls;
    const hasUrls = Array.isArray(urls) ? urls.length > 0 : typeof urls === 'string' ? urls.length > 0 : false;
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {Array.isArray(urls) ? (
                                            ...
                                            <div className="bg-gray-100 text-xs px-3 py-1.5 rounded border border-gray-200 truncate max-w-[200px] transition-all duration-200 hover:bg-gray-50">
                                                {typeof urls === 'string' ? urls.substring(0, 30) : JSON.stringify(urls).substring(0, 30)}
                                            </div>
                                        )}
                                    </div>
                                </div>
